...
maak_figuur <- function(dataset, component, naam, title = NULL, xas = NULL, yas = NULL, linetype = NULL){
  figuur <- dataset %>%
    filter(AquaComponent %in% component) %>%  
    ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = label, linetype = linetype)) +
...
maak_plot <- function(dataset, component, title = NULL, xas = NULL, yas = NULL, linetype = NULL){
  figuur <- dataset %>%
    filter(AquaComponent %in% component) %>%  
    ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = label, linetype = linetype)) +
    scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") + # visualiseert data per maand in format maand-jaar
...
# nitraat
plot_NO3_R <- maak_plot(data_R, "NO3", "Nitraat_randzone", yas = "mg/L")
plot_NO3_ZP <- maak_plot(data_ZP, "NO3", "Nitraat_zonnepanelen", yas = "mg/L")
...
p1 <- data_R %>%
  ...
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
  labs(title = "Helderheid waterkolom randzone") +
  xlab("Datum staalname") + 
  ylab("Meetwaarde (log10)")  + 
  scale_color_discrete("Variabelen") +
  scale_y_log10()
...
p2 <- data_ZP %>%
  ...
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
  labs(title = "Helderheid waterkolom zonnepanelen") +
  xlab("Datum staalname") + 
  ylab("Meetwaarde (log10)")  + 
  scale_color_discrete("Variabelen") +
  scale_y_log10()
...
plot_Nutrienten <- data %>%
  filter(AquaComponent %in% c("NO2", "NO3", "NH4", "TN", "TP", "NO2_N", "NO3_N", "NH4-N")) %>%
  ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = label, linetype = FieldSampleID)) + 
  geom_line() + 
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
...
```{r}
# Voor TN
cum_TN_R <- nutriënten_R %>%
  filter(AquaComponent == "TN") %>%
  arrange(FieldSamplingDate) %>%
  mutate(CumulativeSum = cumsum(ResultCalc))

cum_TN_ZP <- nutriënten_ZP %>%
  filter(AquaComponent == "TN") %>%
  arrange(FieldSamplingDate) %>%
  mutate(CumulativeSum = cumsum(ResultCalc))

p1 <- ggplot(data = cum_TN_R, aes(x = FieldSamplingDate, y = CumulativeSum)) + 
  geom_line(color = "orange") + 
  geom_point(color = "orange") +
  labs(title = "Cumulatieve plot totale stikstof randzone",
       x = "Datum staalname",
       y = "Cumulatieve som van meetwaarde (mg/L)") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y")

p2 <- ggplot(data = cum_TN_ZP, aes(x = FieldSamplingDate, y = CumulativeSum)) + 
  geom_line(color = "orange") + 
  geom_point(color = "orange") +
  labs(title = "Cumulatieve plot totale stikstof zonnepanelen",
       x = "Datum staalname",
       y = "Cumulatieve som van meetwaarde (mg/L)") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y")
...
# voor TP
cum_TP_R <- nutriënten_R %>%
  filter(AquaComponent == "TP") %>%
  arrange(FieldSamplingDate) %>%
  mutate(CumulativeSum = cumsum(ResultCalc))

cum_TP_ZP <- nutriënten_ZP %>%
  filter(AquaComponent == "TP") %>%
  arrange(FieldSamplingDate) %>%
  mutate(CumulativeSum = cumsum(ResultCalc))

p3 <- ggplot(data = cum_TP_R, aes(x = FieldSamplingDate, y = CumulativeSum)) + 
  geom_line(color = "orange") + 
  geom_point(color = "orange") +
  labs(title = "Cumulatieve plot totale fosfor randzone",
       x = "Datum staalname",
       y = "Cumulatieve som van meetwaarde (mg/L)") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y")

p4 <- ggplot(data = cum_TP_ZP, aes(x = FieldSamplingDate, y = CumulativeSum)) + 
  geom_line(color = "orange") + 
  geom_point(color = "orange") +
  labs(title = "Cumulatieve plot totale fosfor zonnepanelen",
       x = "Datum staalname",
       y = "Cumulatieve som van meetwaarde (mg/L)") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y")
...
```{r}
p1 <- data %>%
  filter(AquaComponent %in% c("O2_veld", "O2verz")) %>%
  ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = AquaComponent, linetype = FieldSampleID)) +
  geom_line() + 
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
  labs(title = "Zuurstofgehalte NKP") +
  xlab("Datum staalname") + 
  ylab("Meetwaarde")  + 
  scale_color_discrete("Variabelen") +
  scale_linetype_discrete("Locatie")
...
# zoutparameters
```{r}
# !!! opgelet !!! saliniteit is omgezet van g/kg naar mg/kg (/1000)
p1 <- data %>%
  filter(AquaComponent %in% c("SO4", "SO4_S","Cl", "Na")) %>%
  ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = label, linetype = FieldSampleID)) +
  geom_line() + 
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
  labs(title = "Zoutgehalte NKP") +
  xlab("Datum staalname") + 
  ylab("Meetwaarde (mg/L)")  + 
  scale_color_discrete("Variabelen") +
  scale_linetype_discrete("Locatie")
...
EGV <- data %>%
  filter(AquaComponent %in% c("EGV25_veld", "EGV_calc", "EGV_tit", "EGV_labo")) %>%
  ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = label, linetype = FieldSampleID)) +
  geom_line() + 
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
  labs(title = "Geleidend vermogen NKP") +
  xlab("Datum staalname") + 
  ylab("Meetwaarde (µS/cm)")  + 
  scale_color_discrete("Variabelen") +
  scale_linetype_discrete("Locatie")
EGV
```
