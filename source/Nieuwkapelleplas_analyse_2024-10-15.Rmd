...
plot_helderheid <- data %>% 
  ...
  labs(title = titel_figuur)  +
    scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +
  geom_line()+
  scale_color_discrete(labels = c("chlorofyl_rand (µg/L)", "chlorofyl_zonnepaneel (µg/L)", "Secchi-diepte (m)", "zwevende stof (mg/L)")) + labs(color = "Variabele", x = "Datum", y = "Meetwaarde")  + theme(legend.position = c(0.15,0.7), legend.key.size = unit(2, "cm"))
plot_helderheid

# via functie?
#maak_figuur(data, data$AquaComponent %in% c("chl_a_rand", "chl_a_zp", #"SD_veld", "ZS_105"), "helderheid", "Helderheid waterkolom")
```
