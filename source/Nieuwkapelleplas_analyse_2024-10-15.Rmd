...
```{r}
# maak_figuur creeërt figuren, geen plots, dus grid.arrange werkt niet
plotchloroZP <- maak_figuur(data_ZP, "chl_a", "chlorofyl_a_ZP", "")
plotchloroR <- maak_figuur(data_R, "chl_a", "chlorofyl_a_R")

# maak plots en combineer
titel_figuur = "chlorofyl & secchi-diepte"
plot_helderheid <- data %>% 
    filter(AquaComponent == c("chl_a", "SD_veld", "ZS_105", "ZS_550")) %>% group_by(AquaComponent) %>%
    ggplot(aes(x = FieldSamplingDate, y = ResultCalc, color = FieldSampleID)) + 
    geom_line() +
    geom_point() +
  labs(title = titel_figuur)  +
